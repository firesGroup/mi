   ...
   var $ = layui.jquery,
		...
		switch(eventsName){
			case 'closeCurrent': //关闭当前
			    if (currentTabID > 0) {
			    	...
			    	common.cmsError(' 提示您：默认首页不能关闭的哦！','关闭失败提示');
			    }
				break;
			case 'closeOther': //关闭其他
			    if(ELEM.titleBox.children('li').length>2){
			    	...
			    	common.cmsError(' 提示您：其他选项卡【默认首页】不能关闭！', '关闭失败提示');
			    }else{
			    	common.cmsError(' 提示您：当前无其他可关闭选项卡！', '关闭失败提示');
			    }
				break;
			case 'closeAll': //全部关闭
    ...
    LarryTab.prototype.on = function(events,callback){
		...
		if (typeof(events) !== 'string') {
			common.cmsError('Nav error:事件名配置出错，请参考API文档.', '事件名配置出错')
		}
		...
		if (eventName === 'click'){
		 	if (ELEM.tabCtrlBox.attr('lay-filter') == 'larryOperate'){
		    	...
		    	common.cmsInfo('Nav error:当前传入的lay-filter是'+filter+';而非larryOperate', '常用操作不匹配');
		    }
		}else{
		 	common.cmsInfo(' Nav error:当前传入的事件名是'+eventName+';非点击事件，其他功能稍后续！', '事件名不匹配提示');
		}
    };
